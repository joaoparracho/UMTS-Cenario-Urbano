...
[lattrk, lontrk, arclenInRadians, npts] = sampleGreatCircle(lat1, lon1, lat2, lon2, spacingInDegrees);
...
[vis0, x1, z1, x2, z2] = losprofile(dist, h, oalt, talt, observerAltitudeIsAGL, targetAltitudeIsAGL, apparentradius);

%--------------------------------------------------------------------------

function [lat, lon, arclenInRadians, npts] = sampleGreatCircle(lat1, lon1, lat2, lon2, spacingInDegrees)
...
function [phiTrack, lambdaTrack, arclenInRadians] = doTrack2(phi1, lambda1, phi2, lambda2, npts)
% Interpolate regularly spaced points along a great circle.

[fullArcLength, az] = greatcircleinv(phi1, lambda1, phi2, lambda2);

% for k = 1:numel(npts)
%     arclenInRadians(:,k) = [linspace(0, fullArcLength(k), npts(k)) zeros(1, max(npts) - npts(k))];
% end
arclenInRadians = ((0:max(npts) - 1)' .* (fullArcLength)./(npts - 1));

[phiTrack, lambdaTrack] = greatcirclefwd(phi1, lambda1, az, arclenInRadians);
...
lambda = lambda0 + atan2( sinDelta.*sin(az), cosPhi0.*cosDelta - sinPhi0.*sin(arclen).*cosAz );
